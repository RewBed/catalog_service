...
import { serializeUnknownError } from '../logger/logging.utils';
...
export class OutboxPublisherService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    if (!brokers.length) {
      this.logger.warn(
        'Kafka publisher is enabled, but KAFKA_BROKERS is empty',
      );
      return;
    }
    ...
    this.logger.log(
      `Kafka outbox publisher started (poll=${this.pollIntervalMs}ms, batch=${this.batchSize})`,
    );
  }
