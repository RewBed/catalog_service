...
import { serializeUnknownError } from 'src/core/logger/logging.utils';
...
@Injectable()
export class ImageUpdatedConsumerService
  implements OnModuleInit, OnModuleDestroy
{
  ...
  async onModuleInit(): Promise<void> {
    ...
    const saslMechanism = this.configService.get<
      'plain' | 'scram-sha-256' | 'scram-sha-512'
    >('KAFKA_SASL_MECHANISM', 'plain');
    const username = this.configService
      .get<string>('KAFKA_USERNAME', '')
      .trim();
    const password = this.configService.get<string>('KAFKA_PASSWORD', '');

    if (!brokers.length) {
      this.logger.warn(
        'Image events consumer is enabled, but KAFKA_BROKERS is empty',
      );
      return;
    }
    ...
    if ((username && !password) || (!username && password)) {
      throw new Error(
        'Kafka auth requires both KAFKA_USERNAME and KAFKA_PASSWORD',
      );
    }

    if (!this.topics.length) {
      this.logger.warn(
        'Image events consumer is enabled, but no entity topics configured',
      );
      return;
    }
    ...
    const groupId =
      this.configService
        .get<string>('KAFKA_GROUP_ID_IMAGE_EVENTS', '')
        .trim() || `${clientId}-image-events-consumer`;
  ...
  private async processImageEvent(
    topic: string,
    payload: Buffer | null,
    ...
    if (this.matchesEventType(eventType, 'image.uploaded')) {
      ...
      await this.imageEventsService.handleImageUploaded(
        topic,
        envelope as ImageUploadedEvent,
      );
      return {
        status: 'processed',
        sourceTopic: topic,
    ...
    if (this.matchesEventType(eventType, 'image.deleted')) {
      ...
      await this.imageEventsService.handleImageDeleted(
        topic,
        envelope as ImageDeletedEvent,
      );
      return {
        status: 'processed',
        sourceTopic: topic,
    ...
    if (this.matchesEventType(eventType, 'image.updated')) {
      const result = await this.imageEventsService.processImageUpdated(
        topic,
        payload,
      );
      return {
    ...
    while (true) {
      try {
        ...
        this.logger.warn(
          ...
          'image event temporary error, retry scheduled',
        );
